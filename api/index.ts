...
async function sendEmail(to: string, subject: string, text: string, attachments: any[] = []) {
  ...
  try {
    ...
    if (err.code === 'EAUTH') logToFile("[API] HINT: This is an authentication error. Check your SMTP_USER and SMTP_PASS (App Password).");
...
app.get("/api/admin/test-email", requireAdminAuth, async (req, res) => {
  ...
  const result = await sendEmail(adminEmail, "TEST EMAIL: Converge System", "This is a test email to verify your SMTP configuration is working correctly on Vercel.");
  res.json(result);
});

app.get("/api/admin/diagnostics", requireAdminAuth, (req, res) => {
  const envKeys = Object.keys(process.env).filter(key => key.startsWith('SMTP_') || key.startsWith('SUPABASE_') || key === 'ADMIN_EMAIL' || key === 'VERCEL' || key === 'VERCEL_URL' || key === 'NODE_ENV');
  const diagnostics = {
    VERSION: "7.6",
...
app.post("/api/submit", async (req, res) => {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
    ...
    return res.status(500).json({ error: "CONFIGURATION_ERROR", message: "Supabase environment variables are missing. Please check Vercel Project Settings." });
  }
  if (!name || !email || !results) return res.status(400).json({ error: "Missing required fields" });

  let paymentProduct: Awaited<ReturnType<typeof getPaymentProduct>>;
  try {
    paymentProduct = await getPaymentProduct(product);
  } catch (configErr: any) {
    logToFile(`[API] CONFIGURATION_ERROR resolving payment product for "${product}": ${configErr.message}`);
    return res.status(500).json({ error: "CONFIGURATION_ERROR", message: "Pricing configuration could not be loaded. Please try again shortly, or contact support if this persists." });
  }
  ...
  try {
    ...
    const { data: finalData, error: finalError, status: finalStatus } = await supabase.from('submissions').insert([payload]).select();
    if (finalError) {
      ...
      return res.status(500).json({ error: "DATABASE_REJECTION", message: finalError.message, code: finalError.code, hint: finalError.hint, details: finalError.details || `This error occurred while trying to save ${JSON.stringify(answers).length} characters of data. If this is a 'too long' error, your Supabase column type is likely too small.` });
    }
    if (!finalData || finalData.length === 0) {
      logToFile(`[API] RLS WARNING: Insert returned no data. Status: ${finalStatus}`);
      return res.status(500).json({ error: "RLS_BLOCKED", message: "The database accepted the request but returned no data. This usually means Row Level Security (RLS) is enabled on the 'submissions' table but no 'INSERT' policy exists for anonymous users.", hint: "Go to Supabase -> Authentication -> Policies and add an 'INSERT' and 'SELECT' policy for the 'submissions' table." });
    }
    ...
    try {
      ...
      if (emailResult.success) logToFile(`[API] Admin notification sent successfully.`);
      else logToFile(`[API] Admin notification failed: ${emailResult.error}`);
    } catch (emailErr: any) {
    ...
    if (!res.headersSent) res.status(500).json({ error: "Internal server error", message: error.message });
  }
});
...
app.post('/api/yoco/create-checkout', async (req, res) => {
  ...
  try {
    ...
    if (error || !submission || !submission.payment_token_hash || !constantTimeEqual(sha256(checkoutToken), submission.payment_token_hash)) return res.status(403).json({ error: 'INVALID_PAYMENT_SESSION', message: 'Payment session is invalid.' });
    const product = await getPaymentProduct(submission.product);
    ...
    if (product.amountCents === 0) {
      await supabase.from('submissions').update({ payment_status: 'paid', payment_provider: 'complimentary', paid_at: new Date().toISOString() }).eq('id', submission.id).neq('payment_status', 'paid');
    ...
    const { error: updateError } = await supabase.from('submissions').update({ yoco_checkout_id: checkout.id, payment_status: 'checkout_created', payment_provider: 'yoco' }).eq('id', submission.id).neq('payment_status', 'paid');
...
app.post('/api/yoco/webhook', async (req, res) => {
  ...
  try {
    ...
    const supabase = getSupabase(true);
    const { data: submission, error } = await supabase.from('submissions').select('id,payment_status,payment_amount_cents,payment_currency,yoco_payment_id').eq('yoco_checkout_id', checkoutId).single();
    ...
    const { error: updateError } = await supabase.from('submissions').update({ payment_status: 'paid', payment_provider: 'yoco', yoco_payment_id: paymentId, paid_at: new Date().toISOString() }).eq('id', submission.id).neq('payment_status', 'paid');
    if (updateError) throw updateError;
...
app.post("/api/admin/generate-report", requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    if (!sub.results) throw new Error("Assessment results are missing for this submission.");
    const results = typeof sub.results === 'string' ? JSON.parse(sub.results) : sub.results;
    const product = sub.product;
    const name = sub.name;
    if (!results.mbti) throw new Error("MBTI type is missing from results.");
    let reportPath;
    if (product === 'comprehensive' || product === 'recruiter') {
      const jobData = { jobTitle: sub.job_title, jobEnvironment: sub.job_environment, jobChallenge: sub.job_challenge, jobDescription: sub.job_description };
    ...
    res.status(500).json({ error: "GENERATION_FAILED", message: err.message, stack: process.env.NODE_ENV === 'development' ? err.stack : undefined });
  }
});
...
app.post("/api/admin/send-report", requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    const { data: paymentCheck, error: paymentCheckError } = await paymentGateSupabase.from('submissions').select('payment_status').eq('id', id).single();
    ...
    if (needsRegen) {
      ...
      const supabase = getSupabase(true);
      const { data: sub, error: subError } = await supabase.from('submissions').select('*').eq('id', id).single();
      if (subError || !sub) return res.status(404).json({ error: "Submission not found for regeneration" });
      const results = typeof sub.results === 'string' ? JSON.parse(sub.results) : sub.results;
      const product = sub.product;
...
app.post("/api/admin/mark-paid", requireAdminAuth, async (req, res) => {
  ...
  try {
    const supabase = getSupabase(true);
    const { data, error } = await supabase.from('submissions').update({ payment_status: 'paid', payment_provider: 'manual_eft', paid_at: new Date().toISOString() }).eq('id', id).neq('payment_status', 'paid').select();
    if (error) {
...
app.get("/api/results", requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    const { data, error, status, count } = await supabase.from('submissions').select('*', { count: 'exact' }).order('id', { ascending: false });
    if (error) {
      logToFile(`Supabase Fetch Error: ${JSON.stringify(error)}`);
      return res.status(500).json({ error: error.message, details: error, connectionOk, connectionError });
    }
    ...
    if (connectionError) res.setHeader('X-Connection-Error', connectionError.replace(/\n/g, ' '));
    res.json(data);
  } catch (error: any) {
...
app.patch("/api/results/:id", requireAdminAuth, async (req, res) => {
  ...
  try {
    const supabase = getSupabase(true);
    const { data, error } = await supabase.from('submissions').update(updates).eq('id', Number(id)).select();
    if (error) {
      logToFile(`[API] Supabase Update Error: ${error.message}`);
      throw error;
    }
    if (!data || data.length === 0) return res.status(404).json({ error: 'Submission not found' });
...
app.delete("/api/results/:id", requireAdminAuth, async (req, res) => {
  ...
  try {
    const supabase = getSupabase(true);
    const { data, error } = await supabase.from('submissions').delete().eq('id', Number(id)).select();
    if (error) {
      logToFile(`Supabase Delete Error: ${JSON.stringify(error)}`);
      throw error;
    }
    if (!data || data.length === 0) return res.status(404).json({ error: 'Submission not found or permission denied', details: 'The record was not deleted. Please ensure you have the correct permissions in Supabase (RLS policies) to delete from the submissions table.' });
    ...
    res.status(500).json({ error: 'Failed to delete submission', details: error.message || error.details || String(error) });
  }
});
...
app.get("/api/reports/:filename", async (req, res) => {
  ...
  if (filename !== requestedFilename || !filename.toLowerCase().endsWith('.pdf')) return res.status(400).send('Invalid report filename.');
  const reportUrl = `/api/reports/${filename}`;
  let sub: any;
  try {
    const supabase = getSupabase(true);
    const { data, error } = await supabase.from('submissions').select('*').eq('report_url', reportUrl).single();
  ...
  try {
    ...
    if (newFilename !== filename) await getSupabase(true).from('submissions').update({ report_url: newReportUrl }).eq('id', sub.id);
    if (fs.existsSync(reportPath)) return res.download(reportPath);
