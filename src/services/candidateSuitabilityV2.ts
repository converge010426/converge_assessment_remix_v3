...
function roleText(job: JobData): string {
  return safe(job.jobTitle, 'the target role');
}
...
function drawHeader(doc: PDFKit.PDFDocument): void {
  ...
  doc.moveTo(startX, startY + badgeH + 10).lineTo(545, startY + badgeH + 10).strokeColor(COLORS.gold).lineWidth(0.75).stroke();
}
...
function sectionTitle(doc: PDFKit.PDFDocument, title: string): void {
  doc.fillColor(COLORS.navy).font('Helvetica-Bold').fontSize(17).text(title.toUpperCase(), PAGE.left, 118, { characterSpacing: 1 });
...
function paragraph(doc: PDFKit.PDFDocument, text: string, size = 11.2, gap = 5): void {
  const options = { width: PAGE.width, align: 'justify' as const, lineGap: gap };
  doc.fillColor(COLORS.dark).font('Helvetica').fontSize(size).text(text, PAGE.left, doc.y, options);
  doc.moveDown(0.65);
}
...
function bullet(doc: PDFKit.PDFDocument, text: string, size = 10.5): void {
  ...
  doc.fillColor(COLORS.dark).font('Helvetica').fontSize(size).text(text, 66, startY, { width: 470, lineGap: 3 });
  doc.y += 4;
}
...
function roleFitParagraph(job: JobData, results: AssessmentResults, comps: Competency[]): string {
...
function interviewQuestions(job: JobData, results: AssessmentResults, comps: Competency[]): string[] {
  const role = roleText(job);
...
function managementRecommendations(results: AssessmentResults, comps: Competency[], job: JobData): string[] {
  const s = competencyScores(results);
  const role = roleText(job);
...
export async function generateCandidateSuitabilityV2(name: string, results: AssessmentResults, jobData: JobData = {}): Promise<string> {
  ...
  const comps = buildCompetencies(results);
  const alignmentScore = clamp(avg(...comps.slice(0, 8).map(c => c.score)));
  const alignmentBand = band(alignmentScore);
  const interviewQs = interviewQuestions(jobData, results, comps);
  const recommendations = managementRecommendations(results, comps, jobData);
  ...
  doc.fillColor(COLORS.grey).font('Helvetica-Oblique').fontSize(11.2)
    .text('An integrated decision-support report translating MBTI, EQ and Big Five indicators into role-relevant insight.', 50, 177, { width: 495 });
  ...
  doc.fillColor(COLORS.grey).font('Helvetica-Oblique').fontSize(9.2)
    .text('These indicators are decision-support signals. They are not predictions of job performance and do not replace structured interviewing, references, work samples or professional judgement.', 50, 570, { width: 495, align: 'justify' });
  ...
  doc.fillColor(COLORS.dark).font('Helvetica').fontSize(10.5)
    .text(`To help a recruiter or hiring manager turn the assessment into better questions, clearer role-fit discussion and more informed onboarding decisions for ${name} in relation to ${role}.`, 50, 665, { width: 495, lineGap: 4 });
  ...
  card(doc, 50, doc.y, 240, 82, 'ENVIRONMENT', environment);
  card(doc, 305, doc.y, 240, 82, 'KEY CHALLENGE', challenge);
  doc.y += 103;
  subhead(doc, 'Role Description / Brief');
  paragraph(doc, description, 10.2, 4);
  ...
  profile.forEach(([label, value]) => {
    const y = doc.y;
    doc.fillColor(COLORS.gold).font('Helvetica-Bold').fontSize(8.4).text(label, 50, y, { width: 82 });
    doc.fillColor(COLORS.dark).font('Helvetica').fontSize(10.2).text(value, 140, y, { width: 405 });
    doc.y = y + 24;
  });
  ...
  doc.addPage();
  drawHeader(doc);
  sectionTitle(doc, 'Competency Translation');
  ...
  paragraph(doc, `A ${sentenceCase(alignmentBand.toLowerCase())} across these indicators does not constitute a hiring decision. It means the assessment provides a coherent set of hypotheses for the interview. The recruiter should test the strongest signals for evidence of past performance and the weaker signals for context, compensating behaviours and development potential.`, 10.2, 4);

  // PAGE 4 — ROLE FIT + STRUCTURAL SUPPORT
  doc.addPage();
  drawHeader(doc);
  sectionTitle(doc, 'Role Fit Analysis');
  paragraph(doc, roleFitParagraph(jobData, results, comps), 11.2, 5);
  subhead(doc, 'Where this profile may contribute strongly');
  const strongest = [...comps].sort((a, b) => b.score - a.score).slice(0, 5);
  strongest.forEach(item => bullet(doc, `${item.title}: ${item.explanation}`));
  doc.moveDown(0.4);
  subhead(doc, 'Structural support / risk areas');
  const lower = [...comps].sort((a, b) => a.score - b.score).slice(0, 4);
  lower.forEach(item => bullet(doc, `${item.title}: treat this as an area to validate. Explore the circumstances in which the candidate performs well, the strategies they use to compensate and what the role would require from them.`));
  doc.moveDown(0.4);
  ...
  doc.addPage();
  drawHeader(doc);
  sectionTitle(doc, 'Recommended Interview Questions');
  ...
  interviewQs.forEach((question, index) => {
    const y = doc.y;
    doc.fillColor(COLORS.gold).font('Helvetica-Bold').fontSize(10).text(`${index + 1}`, 50, y, { width: 20 });
    doc.fillColor(COLORS.dark).font('Helvetica').fontSize(10.5).text(question, 78, y, { width: 467, lineGap: 4 });
    doc.y += 8;
  });
  subhead(doc, 'Interview discipline');
  paragraph(doc, 'Use the same core questions and evidence standard for comparable candidates. The assessment should generate better questions, not become a reason to apply a different standard to one candidate.', 10.2, 4);

  // PAGE 6 — ONBOARDING + FINAL VIEW
  doc.addPage();
  drawHeader(doc);
  sectionTitle(doc, 'Onboarding & Management Recommendations');
  ...
  doc.moveDown(0.6);
  doc.rect(50, doc.y, 495, 78).fill(COLORS.light);
  const calloutY = doc.y;
  doc.fillColor(COLORS.navy).font('Helvetica-Bold').fontSize(14).text('BETTER INFORMATION. BETTER QUESTIONS. BETTER-INFORMED DECISIONS.', 68, calloutY + 22, { width: 459, align: 'center', characterSpacing: 0.7 });
  doc.y = calloutY + 92;
  doc.fillColor(COLORS.grey).font('Helvetica-Oblique').fontSize(9.2).text('Strictly Confidential • Candidate Suitability Assessment • Decision-support document', 50, doc.y, { width: 495, align: 'center' });
